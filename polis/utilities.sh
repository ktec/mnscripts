...
ReindexWallet() {
    ...
    polisd -daemon > /dev/null 2>&1
    sleep 5
    echo -e "${GREEN}* Done${NONE}";
    echo 'Waiting for wallet to sync. It will take a while, you can go grab a coffee :)'
    until polis-cli mnsync status | grep -m 1 '"IsBlockchainSynced": true'; do sleep 1 ; done > /dev/null 2>&1
    echo -e "${GREEN}* Blockchain Synced${NONE}";
    until polis-cli mnsync status | grep -m 1 '"IsMasternodeListSynced": true'; do sleep 1 ; done > /dev/null 2>&1
    echo -e "${GREEN}* Masternode List Synced${NONE}";
    until polis-cli mnsync status | grep -m 1 '"IsWinnersListSynced": true'; do sleep 1 ; done > /dev/null 2>&1
    echo -e "${GREEN}* Winners List Synced${NONE}";
    until polis-cli mnsync status | grep -m 1 '"IsSynced": true'; do sleep 1 ; done > /dev/null 2>&1
    echo -e "${GREEN}* Done reindexing wallet${NONE}";
}

EditWalletConfig() {
    sudo nano ~/.poliscore/polis.conf
}
...
echo -e "------------------------------------"
echo -e "| ${CYAN}polisCoin masternode utilities${NONE} |"
echo -e "------------------------------------"
echo
